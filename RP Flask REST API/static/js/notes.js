...
export class Notes {
  constructor() {
    ...
    this.allNoteCards = document.querySelectorAll(".note-card");
  ...
  activateAllControls() {
    this.allNoteCards.forEach((noteCard) => {
...
export class NoteCreateForm {
  ...
  addNoteToList(rawData) {
    ...
    new NoteControl(newNoteCard, data.id);
  }
}
...
class NoteControl {
    ...
    connectNote() {
        let fieldNoteID = this.form.querySelector("input[name='id']");
    ...
    updateNoteInList(rawData, inputForm) {
      ...
      const noteContent = noteCard.querySelector("[data-note-content]");
